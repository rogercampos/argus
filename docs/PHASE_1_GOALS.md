...
- ~~Problems view listing all diagnostics, grouped by file.~~ *(Removed
  2026-06-11 at Roger's request: diagnostics appear only inside the editor
  view — no workspace-wide report, no status-bar counts.)*
...
| 2026-06-11 | Removed the Problems tab and status-bar diagnostic counts (Roger's decision): diagnostics now appear only in the editor (squiggles, error lens, hover). Bottom panel is search-only again. |
| 2026-06-11 | Ruby syntax highlighting via tree-sitter (sourcedelve approach: grammar-based highlighting, ruby-lsp semantic tokens stay off). Vendored tree-sitter-ruby wasm + curated highlight query, viewport-scoped decorations, debounced full reparse (incremental reuse measured 8x slower in wasm). 2ms edit latency on factorial's 16k-line schema.rb. |
